...
for i=1:length(x)
    if x(i)<=beam(1)||x(i)>=beam(2)
       y(i)=0; 
    elseif x(i)>=sensor
       y(i)=((beam(2)-x(i))*(sensor-beam(1))*((beam(2)-beam(1))^2-(beam(2)-x(i))^2-(sensor-beam(1))^2))^2;
    elseif x(i)<sensor
       y(i)=((x(i)-beam(1))*(beam(2)-sensor)*((beam(2)-beam(1))^2-(x(i)-beam(1))^2-(beam(2)-sensor)^2))^2;
    end
%     elseif x(i)>=sensor
